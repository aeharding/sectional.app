...
var Module = {
  ...
  onRuntimeInitialized: function () {
    ...
    GDALGetDriverByName = Module.cwrap("GDALGetDriverByName", "number", [
      "string",
    ]);
    ...
    GDALGetRasterCount = Module.cwrap("GDALGetRasterCount", "number", [
      "number",
    ]);
    GDALGetRasterXSize = Module.cwrap("GDALGetRasterXSize", "number", [
      "number",
    ]);
    GDALGetRasterYSize = Module.cwrap("GDALGetRasterYSize", "number", [
      "number",
    ]);
    ...
    GDALGetRasterStatistics = Module.cwrap(
      "GDALGetRasterStatistics",
      "number",
      [
        "number", // GDALRasterBandH
    ...
    GDALGetRasterNoDataValue = Module.cwrap(
      "GDALGetRasterNoDataValue",
      "number",
      [
        "number", // GDALRasterBandH
    ...
    GDALWarpAppOptionsSetProgress = Module.cwrap(
      "GDALWarpAppOptionsSetProgress",
      "number",
      [
    ...
    GDALTranslateOptionsNew = Module.cwrap(
      "GDALTranslateOptionsNew",
      "number",
      ["number", "number"]
    );
    GDALTranslateOptionsFree = Module.cwrap(
      "GDALTranslateOptionsFree",
      "number",
      ["number"]
    );
    GDALGetProjectionRef = Module.cwrap("GDALGetProjectionRef", "string", [
      "number",
    ]);
    ...
    GDALGetGeoTransform = Module.cwrap("GDALGetGeoTransform", "number", [
      "number",
      "number",
    ]);
    ...
    OSRNewSpatialReference = Module.cwrap("OSRNewSpatialReference", "number", [
      "string",
    ]);
    OSRDestroySpatialReference = Module.cwrap(
      "OSRDestroySpatialReference",
      "number",
      [
    ...
    OCTDestroyCoordinateTransformation = Module.cwrap(
    ...
    OCTTransform = Module.cwrap("OCTTransform", "number", [
      "number",
      "number",
      "number",
      "number",
...
function getProjectedBounds(dataset) {
  ...
  var xCoords = new Float64Array(
    geoCorners.map(function (coords) {
      return coords[0];
    })
  );
...
function getStats(dataset) {
  ...
  noData = GDALGetRasterNoDataValue(
...
function generateTile(tileObj, dataset) {
  ...
  var warpOptions = [
    ...
    "EPSG:4326",
    "-te",
    Math.min(uL.lng, lR.lng).toString(),
    Math.min(uL.lat, lR.lat).toString(),
    Math.max(uL.lng, lR.lng).toString(),
    Math.max(uL.lat, lR.lat).toString(),
    "-ts",
    "256",
    "256",
    "-r",
  ...
  var warpPtrOffset = Module._malloc(
    warpStrPtrs.length * warpStrPtrs.BYTES_PER_ELEMENT
  );
  Module.HEAPU32.set(
    warpStrPtrs,
    warpPtrOffset / warpStrPtrs.BYTES_PER_ELEMENT
  );
  ...
  warpPtrsArray.forEach(function (ptr) {
    Module._free(ptr);
  });
  ...
  if (warpDataset === 0) {
    // In other words, a NULL pointer, indicating an error
  ...
  var pngPtrOffset = Module._malloc(
    pngStrPtrs.length * pngStrPtrs.BYTES_PER_ELEMENT
  );
  ...
  var pngDataset = GDALTranslate(
    pngFilePath,
    warpDataset,
    pngTranslateOptionsPtr,
    null
  );
  GDALTranslateOptionsFree(pngTranslateOptionsPtr);
  Module._free(pngPtrOffset);
  pngPtrsArray.forEach(function (ptr) {
    Module._free(ptr);
  });
  ...
  postMessage({
    tile: {
      request: tileObj,
      bytes: FS.readFile(pngFilePath, { encoding: "binary" }),
    },
  });
